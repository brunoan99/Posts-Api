...
from ..schemas.post import Post, CreatePost, UpdatePost, ReturnPost
...
#TODO TESTS
@router.get("/user/{owner_id}", response_model=List[ReturnPost])
...
#TODO TESTS
@router.get("/latest", response_model=ReturnPost, responses={404: {"model": Message}})
...
#TODO TESTS
@router.get("/latest/user/{owner_id}", response_model=ReturnPost, responses={404: {"model": Message}})
def get_latest_post_from_user(owner_id: int, db: Session = Depends(get_db), search: Optional[str] = "", skip: int = 0):

    post = db.query(models.Post, func.count(models.LikePost.post_id).label("likes")).filter(and_(models.Post.owner_id == owner_id, or_(models.Post.title.contains(search), models.Post.content.contains(search)))).join(models.LikePost, models.Post.id == models.LikePost.post_id, isouter=True).group_by(models.Post.id).order_by(models.Post.created_at.desc()).offset(skip).first()
...
#TODO TESTS
@router.get("/likes", response_model=List[ReturnPost])
...
#TODO TESTS
@router.get("/likes/user/{owner_id}", response_model=List[ReturnPost])
def get_posts_most_likes_from_user(owner_id: int, db: Session = Depends(get_db), limit: Union[None, int] = None, search: Optional[str] = "", skip: int = 0):
    return db.query(models.Post, func.count(models.LikePost.post_id).label("likes")).filter(and_(models.Post.owner_id == owner_id, or_(models.Post.title.contains(search), models.Post.content.contains(search)))).join(models.LikePost, models.Post.id == models.LikePost.post_id, isouter=True).group_by(models.Post.id).order_by(desc("likes"), models.Post.created_at.desc()).offset(skip).limit(limit).all()
...
#TODO TESTS
@router.put("/{post_id}", response_model=Post, responses={404: {"model": Message}, 401: {"model": Message}})
def update_post(post_id: int, body: UpdatePost, db: Session = Depends(get_db), current_user: ReturnUser = Depends(oauth2.get_current_user)):
...
#TODO TESTS
@router.delete("/{post_id}", response_model=Message, responses={404: {"model": Message}, 401: {"model": Message}})
def delete_post(post_id: int, db: Session = Depends(get_db), current_user: ReturnUser = Depends(oauth2.get_current_user)):
